...
$sql="update tblhouses set HouseTitle=:vehicletitle,HouseBrand=:brand,HouseOverview=:vehicleoverview,PricePerDay=:priceperday,FuelType=:fueltype,ModelYear=:modelyear,SeatingCapacity=:seatingcapacity,AirConditioner=:airconditioner,PowerDoorLocks=:powerdoorlocks,AntiLockBrakingSystem=:antilockbrakingsys,BrakeAssist=:brakeassist,PowerSteering=:powersteering,DriverAirbag=:driverairbag,PassengerAirbag=:passengerairbag,PowerWindows=:powerwindow,CDPlayer=:cdplayer,CentralLocking=:centrallocking,CrashSensor=:crashcensor,LeatherSeats=:leatherseats where id=:id ";
...
<head>
	...
	<title>House Rental Portal | Edit House Info</title>
...
<?php 
$id=intval($_GET['id']);
$sql ="SELECT tblhouses.*,tblbrands.BrandName,tblbrands.id as bid from tblhouses join tblbrands on tblbrands.id=tblhouses.HouseBrand where tblhouses.id=:id";
$query = $dbh -> prepare($sql);
$query-> bindParam(':id', $id, PDO::PARAM_STR);
$query->execute();
...
<div class="col-sm-4">
<input type="text" name="vehicletitle" class="form-control" value="<?php echo htmlentities($result->HouseTitle)?>" required>
</div>
...
<div class="col-sm-10">
<textarea class="form-control" name="vehicalorcview" rows="3" required><?php echo htmlentities($result->HouseOverview);?></textarea>
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label">Price Per Month(in USD)<span style="color:red">*</span></label>
<div class="col-sm-4">
...
<option value="Babor Road">Babor Road</option>
<option value="60 fit">60 fit</option>
<option value="Naya Bazar">Naya Bazar</option>
<option value="Lake Road">Lake Road</option>
</select>
</div>
</div>
...
<div class="form-group">
<label class="col-sm-2 control-label">Build Year<span style="color:red">*</span></label>
<div class="col-sm-4">
<input type="text" name="modelyear" class="form-control" value="<?php echo htmlentities($result->ModelYear);?>" required>
</div>
<label class="col-sm-2 control-label">Total Rooms<span style="color:red">*</span></label>
<div class="col-sm-4">
...
<label for="inlineCheckbox2"> Gas/Water Supply </label>
</div>
<?php } else {?>
...
<label for="inlineCheckbox3"> Game Space </label>
</div>
<?php } else {?>
<div class="checkbox checkbox-inline">
...
<label for="inlineCheckbox1"> Yard </label>
</div>
<?php } else {?>
<div class="col-sm-3">
<div class="checkbox checkbox-inline">
...
<label for="inlineCheckbox3"> Multiple Bathroom </label>
</div>
<?php } else { ?>
<div class="checkbox checkbox-inline">
...
<label for="inlineCheckbox1"> Music System </label>
</div>
<?php } else {?>
<div class="checkbox checkbox-inline">
